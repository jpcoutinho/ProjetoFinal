...
static const uint8_t MAX30105_INT_A_FULL_MASK =		(uint8_t)~0b1000000;
...
static const uint8_t MAX30105_INT_PWR_RDY_MASK = (uint8_t)~0b00000001;
...
/**@brief Create timer.
 */
static void create_timerMAX30105()
{
    ret_code_t err_code;
    // Create timers
    err_code = app_timer_create(&timerMAX30105, APP_TIMER_MODE_REPEATED, repeated_timer_handler);
...
MAX30105::MAX30105(const nrfx_twi_t* p_m_twi, uint8_t i2caddr) {
  // Constructor
  _i2caddr = i2caddr;
  _m_twi = p_m_twi;

  create_timerMAX30105();
  app_timer_start(timerMAX30105, APP_TIMER_TICKS(10000), NULL);
}
...
bool MAX30105::begin() {
  ...
  //reseta o chip para uma configuracao padrao
...
void MAX30105::softReset(void) { //FIXFIXFIX Millis
  ...
  while (compareMillis(startTime, millis()) < 100)
  {
...
void MAX30105::setup(uint8_t powerLevel, uint8_t sampleAverage, uint8_t ledMode, int sampleRate, int pulseWidth, int adcRange) {
  ...
  //setFIFOAlmostFull(0x02); //Set to 30 samples to trigger an 'Almost Full' interrupt
...
void MAX30105::test(void){

  nrfx_err_t err_code;

  uint8_t reg = MAX30105_FIFODATA;
  nrfx_twi_tx(_m_twi, MAX30105_ADDRESS, &reg, sizeof(reg), false);
  uint8_t buffer[6];    
  err_code = nrfx_twi_rx(_m_twi, MAX30105_ADDRESS, buffer, sizeof(buffer));
...
int16_t MAX30105::check(void)
{
  ...
  uint8_t readPointer = getReadPointer(); // 0 a 31
  ...
  if (readPointer != writePointer)
  {
...
#if DEBUGH == 1
    printf("ns: %d, rp: %u, wp: %u\n", numberOfSamples, readPointer, writePointer);
#endif

    //We now have the number of readings, now calc bytes to read from FIFO
    //For this example we are just doing Red and IR (3 bytes each)
    int16_t bytesLeftToRead = numberOfSamples * activeLEDs * 3;
    ...
    //Open I2C connection
    uint8_t reg = MAX30105_FIFODATA;
    err_code = nrfx_twi_tx(_m_twi, MAX30105_ADDRESS, &reg, sizeof(reg), false); 
    //APP_ERROR_CHECK(err_code);

    //We may need to read as many as 288 bytes in max30105 or 192 in max30102 so we read in blocks no larger than I2C_BUFFER_LENGTH
    //Se criar uma struct aqui talvez consigamos transportar mais de 8bytes por vez
    while (bytesLeftToRead > 0)
    {
      ...
      #if DEBUGH == 1
      printf("blratt: %u, toGet: %d\n", bytesLeftToRead, toGet);
     

      //Request toGet number of bytes from sensor

      printf("antes -> rp: %u, wp: %u\n", getReadPointer(), getWritePointer());
      #endif

      uint8_t buffer[toGet]; //FIXFIXFIX o buffer pode ter qualquer tamanho! 
      uint16_t i = 0;
      err_code = nrfx_twi_rx(_m_twi, MAX30105_ADDRESS, buffer, sizeof(buffer));
      //APP_ERROR_CHECK(err_code);
...
bool MAX30105::safeCheck(uint8_t maxTimeToCheck)
{
  ...
  while(1)
  {
	if(compareMillis(markTime, millis()) > maxTimeToCheck) return(false);
...
uint8_t MAX30105::readRegister8(uint8_t address, uint8_t reg) {
  ...
  nrfx_err_t err_code;
  uint8_t buffer; //recebe os 8 bits
  
  err_code = nrfx_twi_tx(_m_twi, address, &reg, sizeof(reg), true); //no stop = true
  //APP_ERROR_CHECK(err_code);  
  err_code = nrfx_twi_rx(_m_twi, address, &buffer, sizeof(buffer));
  //APP_ERROR_CHECK(err_code);
  return buffer;
}
...
void MAX30105::writeRegister8(uint8_t address, uint8_t reg, uint8_t value) {
  ...
  uint8_t regBuf[] = {reg, value}; //envia ao endereco do registrador o value
  err_code = nrfx_twi_tx(_m_twi, address, regBuf, sizeof(regBuf), false); 
  //APP_ERROR_CHECK(err_code);
}
